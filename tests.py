...
first_offset = _previous_offset(df1.index[0], index_offsets)
first_offset_as_ts = first_offset.timestamp()
last_offset_as_ts = _previous_offset(df1.index[-1], index_offsets).timestamp()
...
offset_list = pd.period_range(start = first_offset, end = df1.index[-1], freq = index_offsets)
...
fpt = fastparquet.ParquetFile(file)
fpt.statistics
first_offset = _previous_offset(df2.index[0], index_offsets)
first_offset_as_ts = first_offset.timestamp()
last_offset_as_ts = _previous_offset(df2.index[-1], index_offsets).timestamp()
...
df4 = pd.DataFrame({'humidity': [0.4, 0.5],
                    ...
                    index = [pd.Timestamp('2020/01/02 01:59:00'),
                             pd.Timestamp('2020/01/02 01:59:00')])
write(file, df4, index_offsets = index_offsets, drop_duplicates_on = ['humidity', 'location'])
fpt = fastparquet.ParquetFile(file)
df_full = fpt.to_pandas()
...
# ToDo
# test with different index_offset: '1D', '1MS' and '1W-MON'
